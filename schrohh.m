...
appl.bnd = bnd;
clear equ
equ.c = {hcut^2/(2*mhh_ox),hcut^2/(2*mhh_s)};
equ.a ={'energy_h(x,y)','energy_h(x,y)'};% {7.6e-19,'-sin(3.1416/30*1e9*x)*sin(3.1416/30*1e9*y)*1.6e-17'};
equ.ind = [1,2];
appl.equ = equ;
fem.appl{1} = appl;
fem.frame = {'ref'};
...
eigenhh=eigentemp(1:eigen_no);
shi_sq_rawhh=postinterp(fem,'(abs(u)).^2',[xx(:)';yy(:)'],'solnum',1:length(eigenhh));
area=postint(fem,'(abs(u)).^2','solnum',1:length(eigenhh));

for j=1:length(eigenhh)
    shi_sq_rawhh(j,:)=shi_sq_rawhh(j,:)./area(j);
end
...
load Evm.mat Evmin
eigenhh=eigenhh+Evmin;
save shihh.mat shi_sq_rawhh eigenhh 
