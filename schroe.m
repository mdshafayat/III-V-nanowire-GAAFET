...
appl.bnd = bnd;
clear equ
equ.c = {hcut^2/(2*me_ox),hcut^2/(2*me_s)};
equ.a ={'energy_e(x,y)','energy_e(x,y)'};
equ.ind = [1,2];
appl.equ = equ;
fem.appl{1} = appl;
fem.frame = {'ref'};
...
eigene=eigentemp(1:eigen_no);
shi_sq_rawe=postinterp(fem,'(abs(u)).^2',[xx(:)';yy(:)'],'solnum',1:length(eigene));
area=postint(fem,'(abs(u)).^2','solnum',1:length(eigene));

for j=1:length(eigene)
    shi_sq_rawe(j,:)=shi_sq_rawe(j,:)./area(j);
end
...
load Ecm.mat Ecmin
eigene=eigene+Ecmin;
save shie.mat shi_sq_rawe eigene 
